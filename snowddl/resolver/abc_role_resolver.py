...
class AbstractRoleResolver(AbstractResolver):
    ...
    def create_future_grant(self, role_name, grant: FutureGrant):
        self.engine.execute_safe_ddl(
            "GRANT {privilege:r} ON FUTURE {on_plural:r} IN SCHEMA {name:i} TO ROLE {role_name:i}",
            {
                "privilege": grant.privilege,
    ...
    def drop_future_grant(self, role_name, grant: FutureGrant):
        self.engine.execute_safe_ddl(
            "REVOKE {privilege:r} ON FUTURE {on_plural:r} IN SCHEMA {name:i} FROM ROLE {role_name:i}",
            {
                "privilege": grant.privilege,
                "on_plural": grant.on.plural,
                "name": grant.name,
    ...
    def refresh_future_grant(self, role_name, grant: FutureGrant):
        ...
        self.engine.execute_safe_ddl(
            "GRANT {privilege:r} ON ALL {on_plural:r} IN SCHEMA {name:i} TO ROLE {role_name:i}",
            {
                "privilege": grant.privilege,
                "on_plural": grant.on.plural,
                "name": grant.name,
    ...
    def grant_to_future_grant(self, grant: Grant):
        ...
        future_grant = FutureGrant(
            privilege=grant.privilege,
            on=grant.on,
            name=SchemaIdent(grant.name.env_prefix, grant.name.database, grant.name.schema)
