...
class WarehouseResolver(AbstractResolver):
    ...
    def _compare_properties(self, bp: WarehouseBlueprint, row: dict):
        if self._normalise_warehouse_size(bp.size) == self._normalise_warehouse_size(row['size']) \
            ...
            and (row['enable_query_acceleration'] is None or coalesce(bp.enable_query_acceleration, False) == row['enable_query_acceleration']) \
            and (row['query_acceleration_max_scale_factor'] is None or coalesce(bp.query_acceleration_max_scale_factor, 8) == row['query_acceleration_max_scale_factor']):
            return False
    ...
    def _compare_parameters(self, bp: WarehouseBlueprint):
        ...
        query.append("ALTER WAREHOUSE {name:i} SET", {
            "name": bp.full_name,
        })
    ...
    def _build_common_properties(self, bp: WarehouseBlueprint):
        ...
        if self.engine.context.edition >= Edition.ENTERPRISE:
            ...
            query.append_nl("ENABLE_QUERY_ACCELERATION = {enable_query_acceleration:b}", {"enable_query_acceleration": coalesce(bp.enable_query_acceleration, False)})
            query.append_nl("QUERY_ACCELERATION_MAX_SCALE_FACTOR = {query_acceleration_max_scale_factor:d}", {"query_acceleration_max_scale_factor": coalesce(bp.query_acceleration_max_scale_factor, 8)})
    ...
    def _get_existing_warehouse_parameters(self, bp: WarehouseBlueprint):
        ...
        cur = self.engine.execute_meta("SHOW PARAMETERS IN WAREHOUSE {name:i}", {
            "name": bp.full_name,
        })
