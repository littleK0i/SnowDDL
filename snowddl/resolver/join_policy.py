...
from snowddl.blueprint import JoinPolicyBlueprint, ObjectType, Edition, SchemaObjectIdent
from snowddl.resolver.abc_schema_object_resolver import AbstractSchemaObjectResolver, ResolveResult


class JoinPolicyResolver(AbstractSchemaObjectResolver):
    skip_on_empty_blueprints = True
    skip_min_edition = Edition.ENTERPRISE

    def get_object_type(self) -> ObjectType:
        return ObjectType.JOIN_POLICY

    def get_existing_objects_in_schema(self, schema: dict):
        existing_objects = {}

        cur = self.engine.execute_meta(
            "SHOW JOIN POLICIES IN SCHEMA {database:i}.{schema:i}",
            {
    ...
    def get_blueprints(self):
        return self.config.get_blueprints_by_type(JoinPolicyBlueprint)

    def create_object(self, bp: JoinPolicyBlueprint):
        self._create_policy(bp)
        self._apply_policy_refs(bp, skip_existing=True)

        return ResolveResult.CREATE

    def compare_object(self, bp: JoinPolicyBlueprint, row: dict):
        cur = self.engine.execute_meta(
            "DESC JOIN POLICY {full_name:i}",
            {
        ...
        if r["body"] != bp.body:
            self.engine.execute_unsafe_ddl(
                "ALTER JOIN POLICY {full_name:i} SET BODY -> {body:r}",
                {
                    "full_name": bp.full_name,
                    "body": bp.body,
                },
                condition=self.engine.settings.execute_join_policy,
            )
        ...
        if row["comment"] != bp.comment:
            self.engine.execute_unsafe_ddl(
                "ALTER JOIN POLICY {full_name:i} SET COMMENT = {comment}",
                {
                    "full_name": bp.full_name,
                    "comment": bp.comment,
                },
    ...
    def _create_policy(self, bp: JoinPolicyBlueprint):
        query = self.engine.query_builder()

        query.append(
            "CREATE JOIN POLICY {full_name:i} AS ()",
            {
                "full_name": bp.full_name,
            },
        )

        query.append_nl("RETURNS JOIN_CONSTRAINT -> ")
        ...
        self.engine.execute_unsafe_ddl(query, condition=self.engine.settings.execute_join_policy)

    def _drop_policy(self, policy_name: SchemaObjectIdent):
        self.engine.execute_unsafe_ddl(
            "DROP JOIN POLICY {full_name:i}",
            {
    ...
    def _apply_policy_refs(self, bp: JoinPolicyBlueprint, skip_existing=False):
        ...
        for ref in bp.references:
            ...
            if ref_key in existing_policy_refs:
                ...
                # Policy has no columns
                if ref.columns is None and existing_ref_columns is None:
                    continue

                # Policy was applied to exactly the same columns
                if ref.columns and [str(c) for c in ref.columns] == existing_ref_columns:
                    continue

            # Apply new policy
            alter_sql = self.engine.query_builder()

            alter_sql.append(
                "ALTER {object_type:r} {object_name:i} SET JOIN POLICY {policy_name:i}",
                {
            ...
            if ref.columns:
                alter_sql.append(
                    "ALLOWED JOIN KEYS ({columns:i})",
                    {
                        "columns": ref.columns,
                    },
                )

            alter_sql.append("FORCE")

            self.engine.execute_unsafe_ddl(alter_sql, condition=self.engine.settings.execute_join_policy)
            applied_change = True
    ...
    def _get_existing_policy_refs(self, policy_name: SchemaObjectIdent):
        ...
        for r in cur:
            ...
            existing_policy_refs[ref_key] = {
                ...
                "columns": loads(r["REF_ARG_COLUMN_NAMES"]) if r["REF_ARG_COLUMN_NAMES"] else None,
            }
