...
class RowAccessPolicyResolver(AbstractSchemaObjectResolver):
    ...
    def get_existing_objects_in_schema(self, schema: dict):
        ...
        cur = self.engine.execute_meta(
            "SHOW ROW ACCESS POLICIES IN SCHEMA {database:i}.{schema:i}",
            {
    ...
    def compare_object(self, bp: RowAccessPolicyBlueprint, row: dict):
        cur = self.engine.execute_meta(
            "DESC ROW ACCESS POLICY {full_name:i}",
            {
        ...
        if r["body"] != bp.body:
            self.engine.execute_unsafe_ddl(
                "ALTER ROW ACCESS POLICY {full_name:i} SET BODY -> {body:r}",
                {
                    "full_name": bp.full_name,
                    "body": bp.body,
                },
        ...
        if row["comment"] != bp.comment:
            self.engine.execute_unsafe_ddl(
                "ALTER ROW ACCESS POLICY {full_name:i} SET COMMENT = {comment}",
                {
                    "full_name": bp.full_name,
                    "comment": bp.comment,
                },
    ...
    def _create_policy(self, bp: RowAccessPolicyBlueprint):
        ...
        query.append(
            "CREATE ROW ACCESS POLICY {full_name:i} AS (",
            {
                "full_name": bp.full_name,
            },
        )
    ...
    def _drop_policy(self, policy_name: SchemaObjectIdent):
        self.engine.execute_unsafe_ddl(
            "DROP ROW ACCESS POLICY {full_name:i}",
            {
