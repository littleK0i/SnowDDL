...
class WarehouseMonitorRoleResolver(AbstractRoleResolver):
    def get_role_suffix(self):
        return self.config.WAREHOUSE_ACCESS_ROLE_SUFFIX

    def get_role_type(self):
        return self.config.MONITOR_ROLE_TYPE
    ...
    def get_blueprint_monitor_role(self, warehouse: WarehouseBlueprint):
        ...
        bp = RoleBlueprint(
            full_name=build_role_ident(
                self.config.env_prefix, warehouse.full_name.name, self.config.MONITOR_ROLE_TYPE, self.get_role_suffix()
            ),
            grants=grants,
            future_grants=[],
        )
